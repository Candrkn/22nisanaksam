...
def userRegister(request):
    ...
    if request.method == "POST":
        ...
        sifre1 = request.POST.get('sifre1', None)
        sifre2 = request.POST.get('sifre2', None)

        if sifre1 == sifre2:
            if User.objects.filter(email = email).exists():
                messages.error(request, 'Bu mail adresi zaten kullanilmakta')
                return redirect('register')
        
            elif len(sifre1) < 6:
                messages.error(request, "Sifreniz 6 karakterden kisa olamaz!")
                return redirect('register')
        
            elif len(isim) < 6:
                 messages.error(request, "İsim 6 karakterden kisa olamaz!")
                 return redirect('register')
                ...
                messages.success(request, 'Kullanici Olusturuldu! Mail kutunuzu kontrol ediniz!')
...
def userLogin(request):
    ...
    if request.method == "POST":
        ...
        if user is not None:
            ...
            messages.error(request, "Kullanici adi ya da Sifre hatali")
            return redirect('login')
...
def passwordChange(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(request, "Sifre degistirme islemi başarili!")
            return redirect('login')
        else:
...
def account_delete(request):
    ...
    if request.method == "POST":
        if user.is_authenticated:
            ...
            messages.error(request, "Hesabinizi silmek için giriş yapiniz!")
            return redirect('login')
